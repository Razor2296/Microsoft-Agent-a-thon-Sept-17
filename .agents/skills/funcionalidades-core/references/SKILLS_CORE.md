...
* **Grok multimodal / logos diminutos:** Grok responde 400 invalid_image si width*height < 512 (p. ej. logos 24×20 en tesis PDF). _extract_images_from_pdf / _extract_images_from_docx y grok_processor deben pasar por _prepare_image_for_vision_api y **omitir** esas imágenes (no upscale). Umbral: IGNITE_VISION_MIN_IMAGE_PIXELS (default 512). Test: test_vision_min_pixels.py.
* **Anthropic image cap:** Messages API ≤ 10 MiB decoded. Comprimir con `_fit_anthropic_image_bytes`. Env: IGNITE_ANTHROPIC_MAX_IMAGE_BYTES. Test: test_anthropic_image_max_bytes.py.
* **Perplexity / Sonar:** no vision nativo — describe_image + texto; truncar IGNITE_PERPLEXITY_MAX_PROMPT_CHARS; comprimir describe IGNITE_DESCRIBE_IMAGE_MAX_BYTES; mapear 429/context/timeout. Test: test_perplexity_prompt_guard.py.
...
- **Backend Constraints & Safety Rules:**
  - **System Prompt Isolation:** Master prompts, guardrail instructions, and system meta-directives are maintained exclusively server-side and stripped from frontend-facing payloads. RAG recall (`[LOCAL PERSISTENT MEMORY RECALLED]`) and multimodal file assemblies MUST stay model-facing via `_text_for_model_with_rag` / processor internals — never overwrite the user history `content` with `enriched_text` from `generate_response_with_inline_files`. User bubbles show only authored text (+ attachment cards). Sanitize with `strip_model_context_from_user_content` on save/frontend clean.
  ...
  - **Reply-language resolution:** Providers MUST use `resolve_reply_language` / `append_mandatory_reply_language` with the sticky `force_language` from the API. Forbidden: remapping English→Spanish, OS-locale fallback, or call-site language literals.
  - **Shared provider prompting contract (README):** All UI providers (Gemini, DeepSeek, OpenAI, Anthropic, Perplexity, Grok; Alibaba backend) MUST mold to the same app helpers — `append_mandatory_reply_language`, `wrap_user_query_for_language`, `multimodal_describe_prompt`. Text and `generate_response_with_inline_files` share the contract. Privacy/safety refusals follow the reply language. Forbidden: soft “user spoke Spanish” notes only when `force_language` is set, or English-only “Analyze this image…” captions that bias text-only models into English.
...
- **Voice Input & State UI (`audio.js`, `stateMachine.js`, `chat.js`):**
  - **WhatsApp-Style Audio Message Architecture (IMMUTABLE RULE):**
    ...
    - **IMMUTABLE — voice vs PDF hydrate:** `buildMessageAttachmentsHTML` hides `Voice_Message_*`. Therefore `refreshMessageAttachments` MUST early-return (or re-call `buildVoicePlayerHTML`) for voice bubbles; `sendMessage` MUST skip refresh when files are voice-only. Forbidden: empty YOU bubble with only “Tokens Consumed” after mic send. Guard: `dataset.isVoice` + `test_voice_bubble_refresh_guard.py`.
