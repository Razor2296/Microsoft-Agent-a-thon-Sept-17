#!/usr/bin/env bash
# Sync ignite-api on PAYG after CI build. Keep aligned with Deploy-IgniteApiAca.ps1.
set -euo pipefail
...
az_retry az containerapp registry set -g "${RESOURCE_GROUP}" -n "${CONTAINER_APP_NAME}" \
  --server "${ACR_LOGIN_SERVER}" --username "${ACR_USERNAME}" --password "${ACR_PASSWORD}" -o none
az_retry az containerapp identity assign -g "${RESOURCE_GROUP}" -n "${CONTAINER_APP_NAME}" --system-assigned -o none
sleep 12
...
az_retry az containerapp secret set -g "${RESOURCE_GROUP}" -n "${CONTAINER_APP_NAME}" --secrets "${SECRETS[@]}" -o none

SUFFIX="g$(echo "${GITHUB_SHA}" | cut -c1-7)"
az_retry az containerapp update -g "${RESOURCE_GROUP}" -n "${CONTAINER_APP_NAME}" \
  --image "${IMAGE}" \
  --cpu 2.0 --memory 4Gi \
  --set-env-vars \
    "APP_ROLE=api" \
