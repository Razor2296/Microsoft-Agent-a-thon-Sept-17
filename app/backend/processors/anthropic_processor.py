...
class AnthropicChat(BaseChat):
    ...
    def generate_response(
        ...
        if is_spanish:
            system_notes.append(
                "[INSTRUCCIÓN MANDATORIA DE IDIOMA] ¡DEBES RESPONDER ÚNICAMENTE EN ESPAÑOL! El usuario se comunica en español (incluso si usa jerga, modismos o faltas de ortografía como 'shampions'). Ignora el idioma italiano, inglés u otro idioma de cualquier resultado de búsqueda o fuente web. TODA tu respuesta DEBE estar 100% escrita en Español a menos que el usuario solicite explícitamente cambiar de idioma."
            )
        else:
            system_notes.append(
                f"[MANDATORY LANGUAGE INSTRUCTION] You MUST reply ONLY in {lang_name}! Ignore the language of any web search results or scraped context. Reply entirely in {lang_name} UNLESS the user explicitly requests to switch to a different language."
            )
